#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Translate misc-config.sgml to Simplified Chinese."""
import sys, io
...
# Apply translations as (old_fragment, new_fragment) pairs.
# Each old_fragment is a unique substring from the source.
pairs = [
    ("Misc Configuration Parameters", "杂项配置参数"),

    # relcache_expire para
    (
        ...
        '指定关系缓存的过期时间（秒）。关系缓存用于缓存 <productname>PostgreSQL</productname> 系统目录的查询结果，'
        '<productname>Pgpool-II</productname> 通过该缓存获取包括表结构在内的各种信息，并检查表类型（例如，检查被引用的表是否为临时表）。'
        '缓存存储在 <productname>Pgpool-II</productname> 子进程的本地内存空间中，其生命周期与子进程相同。'
        '如果启用了 <xref linkend="guc-enable-shared-relcache">，缓存还会保存在共享内存中以在子进程间共享。'
    ...
    (
        ...
        '指定 relcache 条目的数量。默认为 256。每个表大约创建 10 个缓存条目。'
        '因此可以按「使用表数量 × 10」来估算所需的关系缓存大小。'
    ),
    ...
    (
        ...
        '如果以下消息在 <productname>Pgpool-II</productname> 日志中频繁出现，'
        '则可能需要增大 <varname>relcache_size</varname> 以改善性能。'
    ),
    ...
    (
        ...
        '设置为 on 后，关系缓存将在 <productname>Pgpool-II</productname> 子进程间通过内存查询缓存共享（详见 <xref linkend="runtime-in-memory-query-cache-enabling">）。'
        '默认为 on。每个子进程都需要访问 <productname>PostgreSQL</productname> 的系统目录。'
        '启用此功能后，其他进程可以从查询缓存中提取目录查找结果，从而降低查询频率。'
        '即使修改了系统目录，缓存也不会自动失效，因此强烈建议配合使用 <xref linkend="guc-relcache-expire"> 参数设置基于超时的缓存失效机制。'
    ),
    ...
    (
        ...
        '即使 <xref linkend="guc-memory-cache-enabled"> 为 off，此参数仍可使用。'
        '此时会一并使用某些查询缓存参数（<xref linkend="guc-memqcache-method">、'
        '<xref linkend="guc-memqcache-maxcache"> 及各缓存存储参数）。'
    ),
    ...
    (
        ...
        '<productname>Pgpool-II</productname> 首先搜索本地关系缓存。'
        '如果在本地缓存中未找到，且此功能已启用，则会搜索共享关系查询缓存。'
        '如果在查询缓存中找到，则将其复制到本地关系缓存中。'
        '如果在任何地方都未找到缓存条目，'
        '<productname>Pgpool-II</productname> 将向 <productname>PostgreSQL</productname> 执行查询，'
        '并将结果存储到共享关系缓存和本地缓存中。'
    ),
    ...
    (
        ...
        '创建关系缓存条目时发送查询的目标节点。'
        '如果设置为 <literal>primary</literal>，查询将发送到主节点。'
        '这是默认值，也是大多数用户推荐的选择，因为查询可以获取最新信息。'
        '如果要降低主节点的负载，可以将参数设置为 <literal>load_balance_node</literal>，'
        '将查询发送到负载均衡节点。'
        '对于 <productname>Pgpool-II</productname>/主服务器位于 A 大陆而其他 '
        '<productname>Pgpool-II</productname>/备服务器位于 B 大陆的系统，这一选项尤其有用。'
        'B 大陆的客户端希望从备库读取数据，因为它地理位置更近。'
        '此时可将 backend_weight0（代表主节点）设为 0，backend_weight1（代表备节点）设为 1，'
        '并将 relcache_query_target 设置为 <literal>load_balance_node</literal>。'
    ),
    ...
    (
        ...
        '需要注意的是，如果向备节点发送查询，由于复制延迟，刚创建的表和行在备服务器上可能还不可用。'
        '因此，对于数据修改活动频繁的系统，不建议采用这种配置。'
    ),
    ...
    (
        ...
        '设置为 <literal>catalog</literal> 或 <literal>trace</literal>，'
        '可启用 <acronym>SELECT</acronym> 语句中的临时表检查。'
        '如果指定 <literal>catalog</literal>，'
        '<productname>Pgpool-II</productname> 会查询主 <productname>PostgreSQL</productname> 后端的系统目录来检查临时表，'
        '这会增加主服务器的负载。'
    ),
    ...
    (
        ...
        '如果设置为 <literal>trace</literal>，'
        '<productname>Pgpool-II</productname> 会跟踪临时表的创建和删除操作以获取临时表信息，'
        '因此无需访问系统目录。'
        '但是，如果临时表的创建对 <productname>Pgpool-II</productname> 不可见（例如在函数或触发器中完成），'
        '则 <productname>Pgpool-II</productname> 无法识别临时表的创建。'
    ),
    ...
    (
        ...
        '为兼容 4.0 及更早版本，<productname>Pgpool-II</productname> 接受 <literal>on</literal>（与 <literal>catalog</literal> 相同）'
        '和 <literal>off</literal>（与 <literal>none</literal> 相同），它们可能会在未来的版本中被删除。'
    ),
    ...
    (
        ...
        '如需检查未记录表，<productname>Pgpool-II</productname> 会查询主 <productname>PostgreSQL</productname> 后端的系统目录，'
        '这会增加主服务器的负载。'
        '如果您确定系统中永远不会使用未记录表（例如，您使用的是 9.0 或更早版本的 <productname>PostgreSQL</productname>），'
        '则可以安全地关闭 <varname>check_unlogged_table</varname>。默认为 on。'
    ),
    ...
    (
        ...
        'pid_file_name 路径可以相对于 pgpool.conf 文件位置指定，也可以使用绝对路径。'
        '默认为 <literal>"/var/run/pgpool/pgpool.pid"</literal>。'
    ),
    ...
    # health_check_test - caution (TAB character on these lines)
    (
    ...
    (
        ...
        '     repeating failovers.',
        '设置为 on，可启用健康检查的测试功能。在这种情况下，健康检查进程会查看 <xref linkend="guc-work-dir"> 下的 <filename>backend_down_request</filename> 文件。'
        '该文件可能包含多行，每行对应一个后端。文件中的行以后端 ID 开头（必须是从小数 0 开始的十进制数），接着是一个 TAB，最后以 "down" 结尾。'
        '后端被假定为宕机状态，<productname>Pgpool-II</productname> 将启动故障转移。'
        '故障转移完成后，健康检查进程会将 "down" 改写为 "already_down" 以防止重复故障转移。'
    ),
    ...
    (
        ...
        '此功能对测试 <xref linkend="guc-failover-require-consensus"> 非常有用。假设我们有 3 个 watchdog 节点，'
        '每个 watchdog 检查后端 0 的健康状况。只在 watchdog 0 下的文件中设置 "0\tdown"，'
        '其他 watchdog 将不认同后端 0 不健康，因此不会发生故障转移。'
        '使用此功能可以模拟此类部分网络故障。'
    ),
...
for old, new in pairs:
    if old not in content:
        print(f"ERROR: fragment not found: {repr(old[:80])}")
        errors += 1
    else:
