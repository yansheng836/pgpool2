...
# Store translations in a list of (old, new) tuples
# Use Unicode escapes for quotes to avoid string delimiting issues
q = '\u201c'  # left double quote
qq = '\u201d'  # right double quote
...
pairs.append((
    ...
    '指定关系缓存的过期时间（秒）。关系缓存用于缓存 '
    '<productname>PostgreSQL</productname> 系统目录的查询结果，'
    '<productname>Pgpool-II</productname> 通过该缓存获取包括表结构在内的各种信息，'
    '并检查表类型（例如，检查被引用的表是否为临时表）。'
    '缓存存储在 <productname>Pgpool-II</productname> 子进程的本地内存空间中，'
    '其生命周期与子进程相同。'
    '如果启用了 <xref linkend="guc-enable-shared-relcache">，'
    '缓存还会保存在共享内存中以在子进程间共享。'
    '因此，如果使用 <command>ALTER TABLE</command> 或其他方式修改了表，'
    '关系缓存将变得不一致。'
    '为此，<varname>relcache_expire</varname> 控制缓存的生命周期。'
    '默认值为 0，表示缓存永不过期。'
))
...
for old, new in pairs:
    if old not in content:
        print(f"ERROR: not found: {repr(old[:80])}")
        errors += 1
    else:
