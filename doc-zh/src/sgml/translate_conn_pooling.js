const fs = require('fs');
const path = 'C:/Users/荷塘月色/AppData/Local/Temp/pgpool2-full/doc-zh/src/sgml/connection-pooling.sgml';
...
const replacements = [
  ...
  ['However, connections to ', '但是，即使 <varname>connection_cache</> 为 on，连接到 '],
  [' and ', '、'],
  [' databases are not cached even if', ' 数据库的连接也不会被缓存。'],
  ['You need to restart ', '如果修改此值，需要重启 '],
  [' if you change this value.', '。'],
  ['The maximum number of cached connections\n       in each ', '每个 '],
  [' child\n       process. ', ' 子进程中缓存连接的最大数量。'],
  ...
  ['Default value is 4. Please be aware that the number of\n       connections from ', '默认值为 4。请注意，'],
  [' processes to the backends may reach\n       num_init_children * max_pool in total.', ' 进程到后端的连接数总计可能达到 num_init_children * max_pool。'],
  ['This parameter can only be set at server start.', '此参数只能在服务器启动时设置。'],
  ...
  [' system call.\n       See ', ' 系统调用的 backlog 参数有上限限制。请参阅 '],
  [' for more details.', ' 以了解更多详细信息。'],
  ['Default is 2.', '默认为 2。'],
  ['When set to on, ', '设置为 on 时，'],
  [' enables the serialization\n       on incoming client connections.', ' 会对传入客户端连接启用序列化。'],
  ['Without serialization the OS kernel wakes up all of the ', '未启用序列化时，OS 内核会唤醒所有'],
  [' children processes to execute ', ' 子进程来执行 '],
  ...
  [' process gets woken up\n       for incoming connection to execute the ', ' 进程被唤醒来处理传入连接的 '],
  ['But serialization has its own overheads, and it is recommended\n       to be used only with the larger values of ', '但序列化本身也有开销，建议仅在 '],
  ['.\n       For the small number of ', ' 值较大时使用。对于 '],
  [',\n\tthe serialize accept can degrade the performance because of\n\tserializing overhead.', ' 较小的情况，由于序列化开销，serialize_accept 反而可能降低性能。'],
  ['It is recommended to do a benchmark before deciding whether to use\n       ', '建议在决定是否使用 '],
  [' or not, because the correlation\n       of ', ' 之前进行基准测试，因为 '],
  [' and ', ' 与 '],
  ['\n\tcan be different on different environments.', ' 之间的关系在不同环境下可能有所不同。'],
  ['When ', '当 '],
  [' is enabled, ', ' 启用时，'],
  ['\thas no effect. Make sure that you set ', ' 无效。请确保将 '],
  [' to 0 if you intend\n\t to turn on the ', ' 设为 0，如果你打算开启 '],
  ['.\n\t And if you are worried about ', '。另外，如果你担心 '],
  [' process memory leaks\n\t or whatever potential issue, you could use ', ' 进程内存泄漏或其他潜在问题，可以使用 '],
  [' instead.\n\t  This is purely an implementation limitation and may be removed in the future.',
   ' 代替。这纯粹是实现的限制，未来可能会移除。'],
  ['Default is off.', '默认为 off。'],
  ...
  [' when it\n       is terminated because of ', ' 被终止时，'],
  [".\n       ", ' 会立即生成一个新的子进程。'],
  [' is a measure to prevent the\n       memory leaks and other unexpected errors in ', ' 是用于防止 '],
  ['\n\n       children.', ' 子进程中内存泄漏和其他意外错误的措施。'],
  ...
  ['\n       children from being occupied by a lazy clients or broken TCP/IP\n       connection between client and ',
   ' 子进程被懒惰的客户端或客户端与 '],
  ['\n       between client and ', ' 之间损坏的 TCP/IP 连接占用很有用。'],
  [' is ignored in\n       the second stage of online recovery.', ' 在在线恢复的第二阶段会被忽略。'],
  ['The default is 0, which turns off the feature.', '默认为 0，表示关闭该功能。'],
  ['This parameter can be changed by reloading the ', '此参数可通过重新加载 '],
  [' configurations.', ' 配置来更改。'],
  ['You can also use ', '也可以使用 '],
  [' command to alter the value of\n       this parameter for a current session.', ' 命令来修改当前会话中此参数的值。'],
  ['Specifies the lifetime of a ', '指定 '],
  ['\n       child process in terms of the number of client connections it can receive.\n       ', ' 子进程以客户端连接数为度量的生命周期。'],
  [' will terminate the child process\n       after it has served ', ' 在服务的客户端连接数达到 '],
  [' client connections and will immediately spawn a new child process to take its place.',
   ' 后将终止该子进程，并立即生成一个新的子进程接替其位置。'],
  [' is useful on a very busy server,\n       where ', ' 在非常繁忙的服务器上很有用，此时 '],
  [' and ', ' 和 '],
  ['\n\tnever gets triggered. It is also useful to prevent the ', ' 都不会触发。它也用于防止 '],
  ...
  ['设置较短的值。'],
  ['The default is 0, which means the cached connections will not be\n       disconnected.', '默认为 0，表示缓存连接不会断开。'],
  ['Specifies the ', '指定退出用户会话时发送到后端以重置连接的 '],
  ...
  ['\n       on different ', ' 版本的 '],
  ['\n       Note, however, that ', '\n      注意，'],
  [' command should be always included.', ' 命令应始终包含在内。'],
  ...
  ['Default is ', '默认为 '],
  [' supports two destinations\n       for logging the ', ' 支持两种记录 '],
  [' messages.\n       The supported log destinations are ', ' 消息的日志目标。支持的日志目标为 '],
  [' and ', ' 和 '],
  ...
  ['The default is ', '默认为 '],
  ['This parameter sets the permissions for log files when ', '当 '],
  [' is enabled. The parameter value is expected to be a numeric mode specified in the format\n\t accepted by the ', ' 启用时，此参数设置日志文件的权限。参数值应为 '],
  [' and ', ' 和 '],
  [' system calls.', ' 系统调用接受的格式的数值模式。'],
  ...
  ['Set to zero to disable size-based creation of new log files.',
   '设为 0 可禁用基于大小的日志文件创建。'],
  [' is enabled,\n\t this parameter will cause ', ' 启用时，此参数将使 '],
  ...
  [' is enabled,\n       this parameter determines the ', ' 记录日志时，此参数决定要使用的 '],
  ['\n       "facility" to be used.\n       You can choose from ', ' "facility"。可从 '],
  [', ', '、'],
  [';\n       the default is ', '；默认为 '],
  ...
  [',\n       each adding more fields\n       to displayed messages. ',
   '，每个级别在显示的消息中增加更多字段。'],
  ' excludes the logging of ', ' 不包含 '],
  [',\n       ', '、'],
  ' and ', ' 和 '],
  [' error information.', ' 错误信息。'],
  ['This is a ', '这是一个 '],
  '-style string that is output at the beginning of\n       each log line.', ' 风格的字符串，在每行日志的开头输出。'],
  ['\n       characters begin ', ' 字符开始'],
  ...
  ['log_line_prefix escape options', 'log_line_prefix 转义选项'],
  ['Escape', '转义'],
  ['Effect', '效果'],
  ...
  ['Process name', '进程名称'],
  ['Time stamp without milliseconds', '不含毫秒的时间戳'],
  ['Time stamp with milliseconds', '含毫秒的时间戳'],
  ['Database name', '数据库名称'],
  ['User name', '用户名'],
  ['Log line number for each process', '每个进程的日志行号'],
  ['\'%\' character', '\'%\' 字符'],
  ['application names in various process', '各进程中的应用名称'],
  ['Process type', '进程类型'],
  ['application name', '应用程序名称'],
  ['main', 'main'],
  ['child', 'child'],
  ['streaming replication delay check worker', '流复制延迟检查工作进程'],
  ['sr_check_worker', 'sr_check_worker'],
  ['watchdog heart beat sender', 'watchdog 心跳发送方'],
  ['heart_beat_sender', 'heart_beat_sender'],
  ['watchdog heart beat receiver', 'watchdog 心跳接收方'],
  ['heart_beat_receiver', 'heart_beat_receiver'],
  ['watchdog', 'watchdog'],
  ['watchdog life check', 'watchdog 健康检查'],
  ['life_check', 'life_check'],
  ['follow primary child', '跟随主节点子进程'],
  ['follow_child', 'follow_child'],
  ['watchdog utility', 'watchdog 工具进程'],
  ['watchdog_utility', 'watchdog_utility'],
  ['pcp main', 'pcp main'],
  ['pcp_main', 'pcp_main'],
  ['pcp child', 'pcp child'],
  ['pcp_child', 'pcp_child'],
  ['health check process', '健康检查进程'],
  ['health_check%d (%d is replaced with backend node id)',
   'health_check%d (%d 由后端节点 ID 替换)'],
  ['logger process', '日志记录进程'],
  ...
  ['Recommended setting for ', '的推荐设置'],
  ['\n       on different PostgreSQL versions', '不同 PostgreSQL 版本下'],
  ['\n       Note, however, that ', '注意，'],
  ['\n        is not issued when not in a transaction block for 7.4 or later\n       ', ' 对于 7.4 及更新版本的 PostgreSQL，若非事务块状态则不发出 '],
  ['#for example to log on both syslog and stderr',
   '# 例如同时记录到 syslog 和 stderr'],
];
...
function translateText(text) {
  for (const [oldStr, nw] of replacements) {
    text = text.split(oldStr).join(nw);
  }
  return text;
}
...
while (i < content.length) {
  if (content[i] === '<') {
    ...
    let j = content.indexOf('<', i);
    if (j === -1) {
      result += translateText(content.substring(i));
      i = content.length;
    } else {
      result += translateText(content.substring(i, j));
      i = j;
    }
  }
}
